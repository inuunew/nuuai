...
<title>Checkout - InuuMarket</title>
...
<!-- LINK INPUT (untuk produk seperti sosmed) -->
<div class="card" id="linkBox" style="display:none;">
<label class="label"><i class="fas fa-link"></i> LINK TUJUAN</label>
<div class="input-group"><input type="url" placeholder="https://..." id="linkInput"></div>
</div>
...
if (typeof databaseHiburan !== 'undefined' && databaseHiburan[key]) produk = databaseHiburan[key];
if (typeof databaseSosmed !== 'undefined' && databaseSosmed[key]) produk = databaseSosmed[key];.
...
function updateTotal() {
  ...
  // Total tanpa admin fee
  const totalPrice = subtotalProduk + nominalValue;
  ...
  if (required.includes("voucher")) {
    ...
    voucherInfoContainer.innerHTML = `
      <div class="voucher-message">
        ...
        <div>
          <strong style="display:block; margin-bottom:4px;">Bersifat Voucher</strong>
          <span style="font-size:13px;">Informasi lebih lanjut akan dikonfirmasi oleh admin jika sudah melakukan pembayaran</span>
          <span style="font-size:12px; display:inline-block; margin-top:6px;"><i class="fas fa-check-circle"></i> Kendala? hubungi admin.</span>
        </div>
      </div>
    `;
  } else {
...
linkBox.style.display = required.includes("link") ? "block" : "none";
...
function openInfo() {
  ...
  const content = `
    ...
    <div class="note"><strong><i class="fas fa-lightbulb"></i> Catatan:</strong> Nominal yang dimasukkan akan ditambahkan ke total harga. Hubungi CS jika ada kendala. <a href="https://wa.me/6287714886908" target="_blank"><i class="fab fa-whatsapp"></i> Klik CS WhatsApp</a></div>
  `;
...
function openSheet(){
  ...
  const totalPrice = subtotalProduk + nominalValue; // tanpa admin
  ...
  // Kumpulkan semua data user yang muncul
  let userData = [];
  if(document.getElementById("userId") && userIdBox.style.display !== 'none' && document.getElementById("userId").value) userData.push("ID: " + document.getElementById("userId").value);
  if(document.getElementById("username") && usernameBox.style.display !== 'none' && document.getElementById("username").value) userData.push("Username: " + document.getElementById("username").value);
  if(document.getElementById("phone") && phoneBox.style.display !== 'none' && document.getElementById("phone").value) userData.push("No: " + document.getElementById("phone").value);
  if(required.includes("link") && linkInput.value) userData.push("Link: " + linkInput.value);
  let serverVal = "";
  if(required.includes("server")){
    serverVal = (produk.serverType==="id") ? document.getElementById("serverId").value : document.getElementById("serverRegion").value;
    if(serverVal) userData.push("Server: " + serverVal);
  }
  document.getElementById("sheetUser").innerText = userData.join(" | ") || "Data akan dikirim via WA";
}
function closeSheet(){ document.getElementById("confirmSheet").classList.remove("show"); }

function lanjutWA(){
  closeSheet();
  const waNumber = "6287714886908";
  let serverVal = "";
  if(required.includes("server")){
    serverVal = (produk.serverType==="id") ? document.getElementById("serverId").value : document.getElementById("serverRegion").value;
  }
  let voucherInfoForWA = required.includes("voucher") ? "✅ PRODUK VOUCHER (Kode akan dikirim oleh admin setelah pembayaran lunas)" : "Tidak ada voucher";
  
  const subtotalProduk = basePricePerUnit * currentQty;
  const nominalValue = hasNominal ? extractNominalValue(nominalInput.value) : 0;
  const totalPrice = subtotalProduk + nominalValue;
  
  let nominalInfoForWA = hasNominal ? `Nominal Topup: ${nominalInput.value || "-"} (nilai: ${formatRupiah(nominalValue)})` : "Tidak ada nominal khusus";
  
  let linkInfo = required.includes("link") && linkInput.value ? `Link: ${linkInput.value}` : "";
  ...
  const message = encodeURIComponent(
    ...
    `No. Tujuan: ${document.getElementById("phone")?.value || '-'}\n`+
    `Server: ${serverVal || '-'}\n`+
    `${linkInfo}` +
...
function bayar(){
  ...
  if(required.includes("link") && !linkInput.value){ showWarning("Masukkan link tujuan!"); return; }
  ...
  const invoiceUrl = "invoice.html?" +
    ...
    "&link="+encodeURIComponent(linkInput?.value || '')+
    "&"+voucherParam+
